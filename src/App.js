...
function App() {
  ...
  const fetchTasks = async (token) => {
    const response = await fetch("https://todobackend-ldxr.onrender.com/task", {
  ...
  const deleteTask = async (id) => {
    await fetch(`https://todobackend-ldxr.onrender.com/task/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateTaskStatus = async (id, currentStatus) => {
    ...
    const response = await fetch(
      `https://todobackend-ldxr.onrender.com/task/${id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
